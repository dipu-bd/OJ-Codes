...
pll recur(int p = 0, bool imin = 1, bool imax = 1, bool izero = 1)
{
    if(p == len) return mp(0, 1);

    pll& dp = save[p][imin][imax];
    int& dpc = _save[p][imin][imax];
    ...
    REPE(i, b, e)
    {
        pll r = recur(p + 1, imin && i == b, imax && i == e, izero && i == 0);
        dp.fr += r.fr;
        dp.sc += r.sc;
        if(i == 0 && !izero) dp.fr += r.sc;
    }
...
int main()
{
    ...
    {
        sprintf(num1, "%.10lld", m);
        sprintf(num2, "%.10lld", n);
        printf("%lld\n", recur());
        ++cas;
    }
