...
void precalc()
{
    ...
    for(int i = 2; i < siz; ++i)
    {
        ...
        else
        {
            ...
            flag[i] = 2;
            ++gpsiz;
            group[gpsiz].pb(prime[i - 2]);
            group[gpsiz].pb(prime[i - 1]);
            group[gpsiz].pb(prime[i]);
        }
    }
}
...
int main()
{
    ...
    while(sf2(x, y) == 2 && (x|y))
    {
        ...
        for(int i = 1; i <= gpsiz; ++i)
        {
            if(group[i].front() < x) continue;
            if(group[i].back() > y) continue;
            if(group[i].front() > y) break;

            printf("%d", group[i][0]);
            REP(j, 1, group[i].size())
            printf(" %d", group[i][j]);
            putchar('\n');
        }
    }
