...
char num1[10];
char num2[10];
Result save[10][2][2][2];
bool _save[10][2][2][2];

Result recur(int p = 0, bool imax = 1, bool imin = 1, bool izero = 1)
{
    if(p == len) 
    {
        Result r;
    ...
    Result& dp = save[p][imax][imin][izero];
    bool& dpc = _save[p][imax][imin][izero];
    if(dpc) return dp;
    ...
    int b = imin ? num1[p] - '0' : 0;
    int e = imax ? num2[p] - '0' : 9;
    REPE(i, b, e)
    {
        Result r = recur(p + 1, imax && i == e, imin && i == b, izero && i == 0);
        dp += r;
        if(i || !izero) dp[i] += r.sol;
    }
