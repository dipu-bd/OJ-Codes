...
int n, m;
int save[101][10][1 << 10];
int _save[101][10][1 << 10];

char num[1101];

int recur(int l = 0, int p = 0, int bit = 0)
{
    if(l == m) 
    {
        return (bit + 1 == 1 << n);
    ...
    if(bit)
    {
        ...
        {
            num[l] = '1';
            dp = (dp + recur(l + 1, 1, bit | (1 << 1))) % mod;
        }
        else if(p == n - 1)
        {
            num[l] = p - 1 + '0';
            dp = (dp + recur(l + 1, p - 1, bit | (1 << (p - 1)))) % mod;
        }
        else
        {
            num[l] = p - 1 + '0';
    ...
    {
        //if did not yet started
        num[l] = '0';
        dp = (dp + recur(l + 1, p, bit)) % mod;
        REP(i, 1, n)
        {
            num[l] = i + '0';
            dp = (dp + recur(l + 1, i, bit | (1 << i))) % mod;
        }
    }
