...
char side[6][20] = {"front", "back", "left", "right", "top", "bottom" };

int n;
int cube[510][6];
int save[510][110];

inline int inver(int i)
{
    if(i & 1) return i - 1;
    else return i + 1;
}

int recur(int t, int p)
{
    if(t == n) return 0;
    ...
    REP(i, 0, 6)
    {
        if(p == 0 || cube[t][i] == p)
        {
            dp = max(dp, 1 + recur(t + 1, cube[t][inver(i)]));
        }
    }
...
void pathprint(int t, int p)
{
    ...
    REP(i, 0, 6)
    {
        if(p == 0 || cube[t][i] == p)
        {
            int r = 1 + recur(t + 1, cube[t][inver(i)]);
            if(cur == r)
            {
                printf("%d %s\n", t + 1, side[i]);
                pathprint(t + 1, cube[t][inver(i)]);
                return;
            }
        }
    }
