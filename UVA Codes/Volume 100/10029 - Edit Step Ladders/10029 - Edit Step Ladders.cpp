...
{
    if(abs(wlen[i] - wlen[j]) > 1) return false;

    bool err = 0;
    for(int p = 0; p < wlen[i] || p < wlen[j]; ++p)
    {
        if(word[i][p] != word[j][p])
        {
            if(err) return false;
            err = 1;
        }
...
int main()
{
    ...
    int ans = 0; //need to find the longest path in the graph
