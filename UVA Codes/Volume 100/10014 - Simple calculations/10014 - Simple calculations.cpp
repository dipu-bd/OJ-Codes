...
struct Term
{
    ...
    double a;
    double b;
    Term(double x = 0, double r = 0) : a(x), b(r) { }

    Term operator + (const Term& t) const
    {
        return Term(a + t.a, b + t.b);
    }
    Term operator - (const Term& t) const
    {
        return Term(a - t.a, b - t.b);
...
double solve()
{
    ...
    REPE(i, 2, n)
    {
        arr[i] = (arr[i - 1] + C[i -1]).doub() - arr[i - 2];
    }
    arr[n + 1] = (arr[n] + C[n]).doub() - arr[n - 1];

    return (an - arr[n + 1].b) / arr[n + 1].a;
}
