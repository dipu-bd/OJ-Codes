...
int main()
{
    ...
    while(gets(inp))
    {
        alen = parsenum(inp, aval);
        gets(inp);
        xlen = parsenum(inp, xval);
        ...
        for(int i = 0; i < xlen; ++i)
        {
            if(space) putchar(' ');
            space = true;
            printf("%d", calculate(xval[i], alen));
        }
        putchar('\n');
    }
