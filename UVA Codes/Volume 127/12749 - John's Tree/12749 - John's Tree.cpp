...
template<typename T> T bigmod(T n, T p, T m) { if(!p) return 1; else { T r = sqr(bigmod(n, p >> 1, m)) % m; if(p & 1) r = (r * n) % m; return r; } }
...
int main()
{
    ...
    while(test--)
    {
        ...
        ll ans = -1;
        if(d == 0) ans = 1;
        else if(d == 1) ans = (1 + v) % mod;
        else if(v <= 1) ans = -1;
        else if(v == 2) ans = (1 + v * d) % mod;
        else
        {
            ans = bigmod(v - 1, d, mod) - 1;
            ans = pmod(ans * modinv(v - 2, mod), mod);
            ans = (1 + v * ans) % mod;
        }
