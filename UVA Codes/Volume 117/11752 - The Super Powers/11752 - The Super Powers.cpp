...
const ll high = 18446744073709551615LL;

void backtrack(int p = 1, ll n = 2)
{
    if(p >= prime.size()) return;
    if(4 * log2(n) > log2(high)) return;
    
    backtrack(p + 1, n);
    
    ll r = n * n * n; 
    while(log2(r) + log2(n) <= log2(high))
    {
        r *= n;
        num.ins(r);
        printf("%llu\n", r);
