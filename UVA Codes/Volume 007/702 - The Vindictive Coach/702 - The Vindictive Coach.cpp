...
ull save[MAX][MAX][2];
bool dpcas[MAX][MAX][2];

ull recur(int bot, int top, bool topsel)
{
    ull& dp = save[bot][top][topsel];
    bool& dpc = dpcas[bot][top][topsel];
    if(dpc) return dp;
...
int main()
{
    ...
    while(sf2(n, m) == 2)
    {
        ...
        { 
            if(m == 1)
                way = recur(1, n - 3, false);
            else
                way = recur(m - 1, n - m, false);
        }
