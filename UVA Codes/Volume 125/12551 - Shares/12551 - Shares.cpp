...
void readinp()
{
    ...
    REPE(i, 1, N)
    scanf("%lld %lld", &price[i].fr, &price[i].sc); 
    ...
    REP(i, 0, P)
    {
        ...
        {
            sf2(s, q);
            pack[i].fr += price[s].fr * q;
            pack[i].sc += (price[s].sc - price[s].fr) * q;
        }
        //printf("%d %lld %lld\n", i, pack[i].cost, pack[i].profit);
    } 
}
...
ll makeprofit()
{
    ...
    REP(i, 0, P)
    {
        ...
        loop(it, save)
        {
            ll nc = it->fr + pack[i].fr;
            if(nc <= capital) 
            {
                tmp[nc] = max(tmp[nc], it->sc + pack[i].sc);
            }
        }
        if(pack[i].fr <= capital)
        {
            tmp[pack[i].fr] = max(tmp[pack[i].fr], pack[i].sc);
        }
