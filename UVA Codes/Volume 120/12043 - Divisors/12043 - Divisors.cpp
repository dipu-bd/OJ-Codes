...
void build()
{
    ...
    REP(p, 2, SIZ)
    {
        ...
        {
            flag[i] = 1;
            if(!sigma0[i]) sigma0[i] = 1;
            if(!sigma1u[i]) sigma1u[i] = 1;
            if(!sigma1d[i]) sigma1d[i] = 1;
    ...
    REP(i, 4, SIZ)
    {
        if(flag[i]) sigma1[i] = (sigma1u[i] / sigma1d[i]);
    }
}
