...
int n, k;
ll save[65][101][65][65];
int _save[65][101][65][65];

ll recur(int p = 0, int m = 0, int zero = 0, int one = 0)
{
    if(p == n)
    {
        return (m == 0 && zero == one);
    ...
    ll& dp = save[p][m][zero][one];
    int& dpc = _save[p][m][zero][one];
    if(dpc == cas) return dp;
    dpc = cas;
    dp = 0;

    if(p > 0) dp += recur(p + 1, (m << 1) % k, zero + 1, one);
    dp += recur(p + 1, ((m << 1) + 1) % k, zero, one + 1);
