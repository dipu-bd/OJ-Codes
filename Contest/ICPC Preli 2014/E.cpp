...
int getdat()
{
    int d1 = cum[a1] + a2, d2 = cum[b1] + b2;
    int e1 = a1 + cum[a2], e2 = b1 + cum[b2];

    if(a1 < 1 || a2 < 1 || b1 < 1 || b2 < 1) return 2;

    if(a1 > 12)
    {
        if(b2 > 12) return 2;
        if(a1 > day[a2]) return 2;
        if(b1 > day[b2]) return 2;
        if(e1 >= e2) return 2;
        return 1;
    }
    else if(a2 > 12)
    {
        if(b1 > 12) return 2;
        if(a2 > day[a1]) return 2;
        if(b2 > day[b1]) return 2;
        if(d1 >= d2) return 2;
        return 1;
    }
    else //a1 <= 12 and a2 <= 12
    {
        if(b1 > 12)
        {
            if(b1 > day[b2]) return 2;
...
void print(int i)
{
    if(i == 0) puts("Oh no it is ambiguous!");
    else if(i == 1) puts("Okay got it!");
    else puts("I am sure there is some kinda mistake!");
}
