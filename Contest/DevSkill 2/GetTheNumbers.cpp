...
int n;
vector<string> num;
vector<string> suc, fail;

int match(const string& sn)
{
    int oi = -1;
    REP(x, 0, (int)num.size())
    {
        const string& on = num[x];
...
int main()
{
    ...
    while(test--)
    {
        ...
        {
            scanf(" %[^,], %s", inp, stat);
            if(stat[0] == 'S') suc.push_back(inp);
            else fail.push_back(inp);
        }

        printf("Case: %d\n", cas++);

        puts("Success list");
        REP(i, 0, (int)suc.size())
        {
            int m = match(suc[i]);
            printf("%s : ", suc[i].data());
            if(m == -1) puts("Confused");
            else puts(num[m].data());
        }

        puts("Failed list");
        REP(i, 0, (int)fail.size())
        {
            int m = match(fail[i]);
            printf("%s : ", fail[i].data());
            if(m == -1) puts("Confused");
            else puts(num[m].data());
        }
    }
