...
int main()
{
    ...
    while(test--)
    {
        ...
        minval[1][1] = magrid[1][1];

        REPE(i, 2, r)
        minval[i][1] = min(magrid[i][1], minval[i - 1][1]);

        REPE(j, 2, c)
        minval[1][j] = min(magrid[1][j], minval[1][j - 1]);
...
int main()
{
    ...
    while(test--)
    {
        ...
        {
            //take from top
            int vt = magrid[i][j] + magrid[i - 1][j];
            int mvt = min(minval[i - 1][j], vt);
            //take from left
            int vl = magrid[i][j] + magrid[i][j - 1];
            int mvl = min(minval[i][j - 1], vl);
            //take which matter
            if(vt)
        }
...
*/
            ...
            if(magrid[i][j - 1] > magrid[i - 1][j])
            {
                magrid[i][j] += magrid[i][j - 1];
                minval[i][j] = min(magrid[i][j], minval[i][j - 1]);
            }
            else if(magrid[i][j - 1] < magrid[i - 1][j])
            {
                magrid[i][j] += magrid[i - 1][j];
                minval[i][j] = min(magrid[i][j], minval[i - 1][j]);
            }
            else
            {
                magrid[i][j] += magrid[i - 1][j];
                minval[i][j] = min(minval[i][j - 1], minval[i - 1][j]);
            }
        }
