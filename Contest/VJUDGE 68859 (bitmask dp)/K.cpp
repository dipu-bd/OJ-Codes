...
ll recur(int, int);
ll genall(int, int, int, int);

ll genall(int r, int bit, int c = 0, int bit2 = 0)
{
    if(c == w)
    {
        assert(POPC(bit) == w);
    ...
    if(bit & (1 << c))
    {
        return genall(r, bit, c + 1, bit2);
    }
    else
    {
        ll res = genall(r, bit | (1 << c), c + 1, bit2 | (1 << c));
        if(c + 1 < w && !(bit & (3 << c)))
            res += genall(r, bit | (3 << c), c + 2, bit2);
        return res;
    }
}

ll recur(int r = 0, int bit = 0)
{
    if(r == h)
        return (bit == 0);
