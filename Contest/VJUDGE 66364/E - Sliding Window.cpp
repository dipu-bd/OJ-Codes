...
int main()
{
    ...
    REP(i, 0, k)
    {
        pq1.push(node(arr[i], i));
        pq2.push(node(-arr[i], i));
    }
    ...
    REPE(j, k, n)
    {
        int i = j - k;
        if(i > 0) putchar(' ');
        while(pq2.top().pos < i) pq2.pop();
        printf("%d",  -pq2.top().num);
        pq2.push(node(-arr[j], j));
    }
    putchar('\n');
    ...
    REPE(j, k, n)
    {
        int i = j - k;
        if(i > 0) putchar(' ');
        while(pq1.top().pos < i) pq1.pop();
        printf("%d",  pq1.top());
        pq1.push(node(arr[j], j));
    }
    putchar('\n');
