...
bool exist(int i, int p, bool dif)
{
    for( ; inp[i]; ++i)
    {
        int ch = inp[i] - 'a';
        if(!dif)
        {
            REP(k, 0, 3)
            {
                if(k == ch) continue;
                int q = tree[p].next[k];
                if(q > 0 && exist(i + 1, q, true))
                    return true;
            }
        }
        if(tree[p].next[ch] == 0) return false;
        p = tree[p].next[ch];
    }
    return (dif && tree[p].end);
}
