...
int jump[310];
int cost[310];
map<int, pib> save[310];

pib recur(int p, int t)
{
    if(t == 1) return mp(0, true);
    if(p == n) return mp(oo, false);

    if(save[p].count(t)) return save[p][t];
    ...
    //take
    pib a = recur(p + 1, gcd(t, jump[p]));
    if(a.sc)
    {
        dp.fr = min(dp.fr, cost[p] + a.fr);
        dp.sc = true;
    }
...
int main()
{
    ...
    REP(i, 0, n)
    {
        pib a = recur(i, jump[i]);
        if(a.sc)
        {
            res.fr = min(res.fr, cost[i] + a.fr);
            res.sc = true;
        }
    }
