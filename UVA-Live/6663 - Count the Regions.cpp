...
{
    if(i < 0 || j < 0 || i > li || j > lj || grid[i][j]) return;
    grid[i][j] = 1;
    dfs(i, j + 1);
    dfs(i, j - 1);
    dfs(i + 1, j);
...
int main()
{
    ...
    while(sf1(n) == 1 && n)
    {
        ...
        REP(i, 0, n)
        {
            sf4(rect[i].l, rect[i].t, rect[i].r, rect[i].b);
            x[rect[i].l] = 0;
            y[rect[i].b] = 0;
            x[rect[i].r] = 0;
            y[rect[i].t] = 0;
        ...
        li = 1, lj = 1;
        loop(i, x) i->sc = li, li += 2;
        loop(i, y) i->sc = lj, lj += 2;
        ...
        REP(i, 0, n)
        {
            ...
            REPE(p, rect[i].l, rect[i].r)
            {
                grid[p][rect[i].b] = 1;
                grid[p][rect[i].t] = 1;
            }
            REPE(q, rect[i].b, rect[i].t)
            {
                grid[rect[i].l][q] = 1;
                grid[rect[i].r][q] = 1;
            }
