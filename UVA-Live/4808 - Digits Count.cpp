...
FREQ recur(int p = 0, bool ismax = 1, bool countzero = 0)
{
    if(!num[p])
    {
        FREQ f;
    ...
    FREQ& dp = save[p][ismax][countzero];
    bool& dpc = calc[p][ismax][countzero];
    if(dpc) return dp;
    ...
    int b = 0;
    int e = ismax ? num[p] - '0' : 9;
    REPE(i, b, e)
    {
        FREQ f = recur(p + 1, ismax && i == e, i || countzero);

        dp.sol += f.sol;
        REPE(j, 0, 9) dp[j] += f[j];
        if(i > 0) dp[i] += f.sol;
        else if(countzero) dp[i] += f.sol;
    }
