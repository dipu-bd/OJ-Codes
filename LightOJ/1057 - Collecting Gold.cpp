...
{
    int x = abs(pos[p].fr - pos[q].fr);
    int y = abs(pos[p].sc - pos[q].sc);
    return max(x, y);
}

int recur(int bit = 1, int p = 0)
{
    int& dp = save[bit][p];
    int& dpc = dpcase[bit][p];
    ...
    REP(i, 1, pos.size())
    {
        ...
        {
            fin = 0;
            int r = recur(bit | (1 << i), i);
            r += getdis(p, i);
            if(r < dp) dp = r;
        }
    }

    if(fin) dp = getdis(p, 0);
    return dp;
}
