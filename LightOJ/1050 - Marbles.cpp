...
bool _save[502][502][2];
double save[502][502][2];

double recur(int r, int b, int turn = 0)
{
    if(b == 0) return 0.0;
    if(r == 0) return 1.0;

    double& dp = save[r][b][turn];
    bool& dpc = _save[r][b][turn];
    if(dpc) return dp;

    if(turn)
    {
        dp = recur(r, b - 1, turn ^ 1);
    }
    else
    {
        dp = r * recur(r - 1, b, turn ^ 1);
        dp += b * recur(r, b - 1, turn ^ 1);
        dp /= (r + b);
    }
