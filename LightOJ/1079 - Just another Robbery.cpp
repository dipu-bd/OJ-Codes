    ...
    while(test--)
    {
        ...
        REP(i, 0, n)
        {
            RREPE(j, mxl, money[i])
            {
                ...
                if(check[m])
                {
                    if(check[j]) total[j] = min(total[j], total[m] + (1 - total[m]) * prob[i]);
                    else { check[j] = 1; total[j] = total[m] + (1 - total[m]) * prob[i]; }
                }
            }
        }

//        REPE(i, 0, mxl) printf("%7d", i); puts("");
//        REPE(i, 0, mxl) printf("%7d", check[i]); puts("");
//        REPE(i, 0, mxl) printf("%7.3lf", total[i]); puts("");

        int res = 0;
        RREPE(i, 10000, 0)
        {
            if(check[i] && total[i] <= p)
            {
                res = i;
                break;
            }
