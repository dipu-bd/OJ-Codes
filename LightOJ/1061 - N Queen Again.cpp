...
bool canplace(int r, int c)
{
    REP(i, 0, r)
    {
        if(row[config][i] == c) return false;
        if(abs(r - i) == abs(c - row[config][i])) return false;
    }
    return true;
}
...
void backtrack(int r = 0)
{
    ...
    {
        ++config;
        memcpy(row[config], row[config - 1], sizeof(row[config]));
        return;
    }
...
int getcost(int q, int bit)
{
    int x = POPC(bit);
    int y = row[cur][x];
    if(x == queen[q].fr && y == queen[q].sc) return 0;
    if(x == queen[q].fr || y == queen[q].sc) return 1;
    if(abs(x - queen[q].fr) == abs(y - queen[q].sc)) return 1;
    return 2;
}
...
int recur(int bit = 0)
{
    ...
    if(!(bit & (1 << i)))
    {
        int cost = getcost(i, bit);
        dp = min(dp, cost + recur(bit | (1 << i)));
    }
