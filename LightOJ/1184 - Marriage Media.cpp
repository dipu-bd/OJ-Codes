...
bool canmarry(int i, int j)
{
    if(abs(man[i].tall - woman[j].tall) > 12) return false;
    if(abs(man[i].age - woman[j].age) > 5) return false;
    return (man[i].div == woman[j].div);
}
...
int main()
{
    ...
    while(test--)
    {
        ...
        REP(i, 0, m)
        {
            sf3(man[i].tall, man[i].age, man[i].div);
        }
        REP(i, 0, n)
        {
            sf3(woman[i].tall, woman[i].age, woman[i].div);
        }
