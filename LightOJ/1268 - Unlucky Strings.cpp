...
inline void cross(uint M[SZ][SZ])
{
    REP(i, 0, dim)
    REP(j, 0, dim)
    {
...
inline void build_lps()
{
    pi[1] = 0;
    int k = 0;
    REPE(q, 2, dim)
    {
        while(k > 0 && bad[k + 1] != bad[q]) k = pi[k];
        if(bad[k + 1] == bad[q]) ++k;
        pi[q] = k;
    }
}
...
inline void init()
{
    ...
    REP(i, 0, dim)
    {
        for(int j = 1; allow[j]; ++j)
        {
            int q = i;
            while(q > 0 && bad[q + 1] != allow[j]) q = pi[q];
            if(bad[q + 1] == allow[j]) ++q;
            mat[i][q]++;
        }
    }
}
