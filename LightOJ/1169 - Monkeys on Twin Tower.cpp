...
int main()
{
    ...
    while(test--)
    {
        ...
        REP(i, 0, n) sf1(fruit[0][i]);
        REP(i, 0, n) sf1(fruit[1][i]);
        REP(i, 1, n) sf1(jump[0][i - 1]);
        REP(i, 1, n) sf1(jump[1][i - 1]);

        save[0][n] = 0;
        save[1][n] = 0;
        RREPE(p, n - 1, 0)
        {
            save[0][p] = fruit[0][p] + min(save[0][p + 1],
                                           save[1][p + 1] + jump[0][p]);

            save[1][p] = fruit[1][p] + min(save[1][p + 1],
                                           save[0][p + 1] + jump[1][p]);
        }

        int ans = min(save[0][0], save[1][0]);
        printf("Case %d: %d\n", cas++, ans);
    }
