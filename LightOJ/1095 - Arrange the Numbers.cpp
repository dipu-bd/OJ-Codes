...
void NCR()
{
    ...
    REP(r, 0, SIZ)
    {
        ...
        else if(n > r) ncr[n][r] = (ncr[n - 1][r] + ncr[n - 1][r - 1]) % mod;
    }
}
...
ll recur(int a, int b)
{
    ...
    dpc = true;
    if(a == 0 && b == 0) dp = 1;
    else if(b == 0) dp = (a * recur(a - 1, b)) % mod;
    else if(b == 1) dp = (a * recur(a, b - 1)) % mod;
    else dp = (a * recur(a, b - 1) + (b - 1) * recur(a + 1, b - 2)) % mod;
...
ll solve()
{
    return (ncr[m][k] * recur(n - m, m - k)) % mod;
}
