...
ll calculate_x(int x)
{
    ll r1 = 0;
    REP(i, 0, q)
    r1 += (ll)abs(pos[i].x - x) * pos[i].w;
    return r1;
}

ll calculate_y(int y)
{
    ll r1 = 0;
    REP(i, 0, q)
    r1 += (ll)abs(pos[i].y - y) * pos[i].w;
    return r1;
}

int ternary_search_x()
{
    int left, right;
    int high = m, low = 1;
    ll res1, res2;

    for(int pass = 0; pass < 70 && low < high; ++pass)
    {
        left = (2 * low + high) / 3;
        right = (low + 2 * high) / 3;
        res1 = calculate_x(left);
        res2 = calculate_x(right);
        if(res1 == 0) return left;
        if(res2 == 0) return right;
        if(res1 < res2)
            high = right - 1;
        else if(res2 < res1)
            low  = left + 1;
    ...
    while(low <= high)
    {
        res1 = calculate_x(low);
        if(res1 < res2) { res2 = res1; left = low; }
        ++low;
    }
    return left;
}

int ternary_search_y()
{
    int left, right;
    int high = n, low = 1;
    ll res1, res2;
    ...
    for(int pass = 0; pass < 70 && low < high; ++pass)
    {
        left = (2 * low + high) / 3;
        right = (low + 2 * high) / 3;
        res1 = calculate_y(left);
        res2 = calculate_y(right);
        if(res1 == 0) return left;
        if(res2 == 0) return right;
        if(res1 < res2)
            high = right - 1;
...
int main()
{
    ...
    while(test--)
    {
        ...
        REP(i, 0, q)
        sf3(pos[i].x, pos[i].y, pos[i].w);

        int x = ternary_search_x();
        int y = ternary_search_y();
