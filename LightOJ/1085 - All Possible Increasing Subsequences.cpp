...
int getsum(int nod, int b, int e, int x, int y)
{
    if(x <= b && e <= y) return tree[nod];
    ...
    int res = 0;
    if(x <= m) res = getsum(l, b, m, x, y);
    if(y > m) res += getsum(r, 1 + m, e, x, y);

    return (res % mod);
}

void update(int nod, int b, int e, int p, int sum)
{
    if(b == e)
    {
        tree[nod] = (tree[nod] + sum) % mod;
        return;
    }
    ...
    if(p <= m) update(l, b, m, p, sum);
    else update(r, 1 + m, e, p, sum);

    tree[nod] = (tree[l] + tree[r]) % mod;
}
...
int main()
{
    ...
    while(test--)
    {
        ...
        stable_sort(allof(cnv), comp);

        indx[cnv[0].sc] = 1;
        REP(i, 1, n)
        {
            if(cnv[i].fr == cnv[i - 1].fr)
                indx[cnv[i].sc] = indx[cnv[i - 1].sc];
            else
                indx[cnv[i].sc] = indx[cnv[i - 1].sc] + 1;
        }
        ...
        REPE(i, 1, n)
        {
            int sum = 1;
            int p = indx[i];
            if(p > 1) sum += getsum(1, 1, n, 1,  p - 1);
            update(1, 1, n, p, sum % mod);
            res = (res + sum) % mod;
        }
