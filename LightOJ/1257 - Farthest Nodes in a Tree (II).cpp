...
void recur_bottom(int u, int p = -1)
{
    loop(it, graph[u])
    if(it->fr != p)
    {
        recur_bottom(it->fr, u);
        bottom[u] = max(bottom[u], bottom[it->fr] + it->sc);
    }
}

int mxbottom(int u, int p, int bad)
{
    int val = 0;
    loop(it, graph[u])
    if(it->fr != p && it->fr != bad)
    {
        val = max(val, bottom[it->fr] + it->sc);
    }
    return val;
}

void recur_up(int u, int p = -1)
{
    loop(it, graph[u])
    if(it->fr != p)
    {
        up[it->fr] = it->sc + max(up[u], mxbottom(u, p, it->fr));
        recur_up(it->fr, u);
    }
}
