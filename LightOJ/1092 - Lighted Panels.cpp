...
inline void apply(int c, int& prv, int& cur, int& nxt)
{
    prv ^= 1 << c;
    cur ^= 1 << c;
    nxt ^= 1 << c;
...
int recur(int r, int prev, int cur)
{
    if(r == n)
    {
        if(prev) return oo;
    ...
    int& dp = save[r][prev][cur];
    int& dpc = _save[r][prev][cur];
    if(dpc == cas) return dp;

    dp = oo;
    dpc = cas;

    REP(bit, 0, (1 << m)) //for all subset
    {
        int t = cur;
        int p = prev;
        int nxt = bits[r + 1];

        REP(c, 0, m) //apply this subset by toggling
        if(bit & (1 << c))
            apply(c, p, t, nxt);

        if(r == 0 || p == 0) //if prev has all light on
            dp = min(dp, recur(r + 1, t, nxt) + POPC(bit));
    }
