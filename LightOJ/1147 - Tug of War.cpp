...
int total;
int arr[110];
bool make[101][100005];

bool recur(int p, int sum = 0)
{
    ...
    while(test--)
    {
        ...
        REP(i, 0, n)
        {
            RREPE(s, half, arr[i])
            {
                if(make[s - arr[i]])
                {
                    int p = taken[s - arr[i]] + 1;
                    make[s] = true;
                    int a = abs(n - (taken[s] << 1));
                    int b = abs(n - (p << 1));
                    if(a > b) taken[s] = p;
                }
            }
...
//        REPE(i, 0, half) printf("%3d", i); puts("");
//        REPE(i, 0, half) printf("%3d", make[i]); puts("");
//        REPE(i, 0, half) printf("%3d", taken[i]); puts("");

        int ans1 = 0;
        RREPE(s, half, 0)
        {
            if(make[s] && abs((taken[s] << 1) - n) <= 1)
            {
                ans1 = s;
                break;
            }
